...
handleButtonClick = (state) => {
  ...
  {
    this.props.history.push('/admin/edit_job/'+state.target.id);
  }
  else if(state.target.value==='delete')
  {
        //this.handleDelete(state.target.id);

        this.props.handleDelete_hoc({'title_id':state.target.id},'delete_job_title','job title').then(data => {
          if(data.status==200)
          {
...
{
    this.props.handleStatusUpdate_hoc({'a_id':status_id,'value':update_value},'update_status_jobtitle','Job title').then(data => {
      var new_obj = {};
...
add_job_url= ()=>{
    this.props.history.push('/admin/add_job');
}

async serverside_pagination_logic(new_obj,api_name='')
{
    //console.log('helloooo ---> '+api_name);
    ...
    this.props.serverside_pagination_api(new_obj,(api_name=='')?'list_job_title_pagination':api_name).then(response => {
...
async reset_search_filter()
{
  ...
  await this.setState({search_name:'', data:[],totalRows:0,reset_pagination:true});
  var new_obj = {};
  new_obj.perPage = 10;
  new_obj.page = 0;
      ...
      <div className="animated fadeIn">
          <Row>
            <Col xs="12" md="12">
              <Card>
                  <CardHeader>
                    <strong>Search</strong> <span className="float-right"><Button block color="primary" onClick={this.add_job_url} className="btn-pill">Add Job Title</Button></span>
                  </CardHeader>
                <CardBody>
...
export default HOCdatatable(Listjob);