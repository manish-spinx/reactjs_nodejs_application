...
submitForm(e){
     ...
     {     
          ...
          const response_check = api_service('update_user',formdata,'2');
            response_check.then(response=>{
                  ...
                  {
                    toast.success('User Profile Update Successfully',{ autoClose: 1000 });
                            setTimeout(() => {
 ...
 componentDidMount()
 {
      const response_check = api_service('edit_user',{'user_id':localStorage.getItem("login_user_id")});
      response_check.then(response=>{
            ...
            if(response.status==200)
            {
                  ...
                  localStorage.setItem("user_profile_link",obj_value.profile_image_link);
          ...
          <Col xs="12" md="12">
            <Card>
              ...
              <CardBody>
                ...
                <Button color="primary" type="submit" className="mr-2 px-4" disabled={this.state.disabled}>{this.state.disabled ? 'Updating...' : 'Update Profile'}</Button>
