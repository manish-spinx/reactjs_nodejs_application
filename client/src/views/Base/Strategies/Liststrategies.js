...
class Liststrategies extends Component {
...
handleButtonClick = (state) => 
{
  ...
  {
    this.props.history.push('/admin/edit_strategies/'+state.target.id);
  }
  else if(state.target.value==='delete')
  {
    this.props.handleDelete_hoc({'s_id':state.target.id},'delete_strategies','strategy').then(data => {
           if(data.status==200)
           {
...
{
  this.props.handleStatusUpdate_hoc({'s_id':status_id,'value':update_value},'update_status_strategies','Strategy').then(data => {
        var new_obj = {};
...
add_job_url= ()=>{
  this.props.history.push('/admin/add_strategies');
}
...
async serverside_pagination_logic(new_obj,api_name='')
{
    ...
    new_obj.c_field_name = this.state.c_field_name;
      ...
      this.props.serverside_pagination_api(new_obj,(api_name=='')?'list_strategies_pagination':api_name).then(response => {            
         ...
         <div>
            ...
            <div className="animated fadeIn">
                <Row>
                    ...
                    <Card>
                        <CardHeader>
                            <strong>Search</strong> <span className="float-right"><Button block color="primary" onClick={this.add_job_url} className="btn-pill">Add Strategies</Button></span>
                        </CardHeader>
                        <CardBody>
...
export default HOCdatatable(Liststrategies);